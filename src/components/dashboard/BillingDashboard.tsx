...
import { DollarSign, FileText, TrendingUp, AlertCircle, Clock, CheckCircle2 } from "lucide-react";
...
export function BillingDashboard() {
  ...
  const loadBillingStats = async () => {
    try {
      const [allInvoices, pending, paid] = await Promise.all([
        supabase.from("invoices").select("total_amount, balance_due"),
        supabase
          .from("invoices")
      ...
      const revenue = allInvoices.data?.reduce((sum, inv) => sum + Number(inv.total_amount || 0), 0) || 0;
      const outstanding = allInvoices.data?.reduce((sum, inv) => sum + Number(inv.balance_due || 0), 0) || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-l-4 border-l-red-500">
          ...
          <CardContent>
            <div className="text-3xl font-bold">${stats.outstandingBalance.toLocaleString()}</div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <Card className="cursor-pointer hover:shadow-lg transition-all border-l-4 border-l-green-500" onClick={() => navigate("/billing")}>
          <CardHeader>
        ...
        <Card className="cursor-pointer hover:shadow-lg transition-all border-l-4 border-l-blue-500" onClick={() => navigate("/billing")}>
          <CardHeader>
            <div className="flex items-center gap-2">
              <DollarSign className="h-8 w-8 text-blue-500" />
              <div>
                <CardTitle>Record Payment</CardTitle>
                <CardDescription>Process patient payments</CardDescription>
              </div>
            </div>
          </CardHeader>
        </Card>
